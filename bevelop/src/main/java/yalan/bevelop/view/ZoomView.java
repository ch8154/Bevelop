...
public class ZoomView extends FrameLayout {
    ...
    private float doubleTapZoom = 2f;
    ...
    public void setDoubleTapZoom(float doubleTapZoom) {
    ...
    public void smoothZoomTo(final float zoom, final float x, final float y) {
        smoothZoom = clamp(0.2f, zoom, maxZoom);
        smoothZoomX = x;
        smoothZoomY = y;
        if (listener != null) {
    ...
    private void processSingleTouchEvent(final MotionEvent ev) {
        ...
        if (showMinimap && smoothZoom > minZoom && touchingMiniMap) {
                ...
                if (l < 30.0f) {
                    ...
                    if (System.currentTimeMillis() - lastTapTime < 500) {
                        if (smoothZoom == minZoom) {
                            float nextZoom = doubleTapZoom;
                            if (nextZoom > maxZoom) {
                                nextZoom = maxZoom;
                            }
                            smoothZoomTo(nextZoom, x, y);
                        } else if (smoothZoom != maxZoom && smoothZoom != minZoom) {
                            float nextZoom = smoothZoom + doubleTapZoom;
                            if (nextZoom > maxZoom) {
                                nextZoom = maxZoom;
                            }
                            smoothZoomTo(nextZoom, x, y);
                        } else if (smoothZoom == maxZoom) {
                            smoothZoomTo(minZoom, getWidth() / 2.0f, getHeight() / 2.0f);
                        }
                        lastTapTime = 0;
                        ev.setAction(MotionEvent.ACTION_CANCEL);
                        super.dispatchTouchEvent(ev);
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (pinching || ld > 30.0f) {
                    ...
                    smoothZoomTo(Math.max(minZoom, zoom * d / (d - dd)), zoomX - dxk / zoom, zoomY - dyk / zoom);
                }
